...
class MeshCoreBleClient extends MeshtasticClient {
  ...
  void _onFrameReceived(Uint8List frameData) {
    ...
    if (frameData.isNotEmpty) {
      ...
      if (cmd == MeshCoreConstants.respDeviceInfo) {
      ...
      } else if (cmd == MeshCoreConstants.respCodeChannelMsgRecv ||
          cmd == MeshCoreConstants.respCodeChannelMsgRecvV3) {
        _log(
          'Received channel message notification (0x${cmd.toRadixString(16)})',
        );

        // Parse channel message response
        // Format for V3 (0x11):
        // [0] response_code (0x11)
        // [1] SNR (int8)
        // [2-3] reserved
        // [4] channel_idx
        // [5] path_len
        // [6] txt_type
        // [7-10] timestamp (uint32 little-endian)
        // [11+] message text
        //
        // Format for V1 (0x08):
        // [0] response_code (0x08)
        // [1] channel_idx
        // [2] path_len
        // [3] txt_type
        // [4-7] timestamp
        ...
        try {
          ...
          if (cmd == MeshCoreConstants.respCodeChannelMsgRecvV3) {
            // V3 format
            if (frameData.length < 11) {
              _log(
                'Channel message frame too short: ${frameData.length}',
                level: 'warn',
              );
              return;
            }
            snr = frameData[offset++]; // SNR
            offset += 2; // Skip reserved bytes
            channelIdx = frameData[offset++];
            pathLen = frameData[offset++];
            txtType = frameData[offset++];
          ...
          final messageBytes = frameData.sublist(offset);
          final message = String.fromCharCodes(messageBytes);

          _log('Channel message: channel=$channelIdx, msg="$message"');

          // Emit as mesh packet event
          // Use a special node ID for channel messages (e.g., 0xFFFFFFFF for broadcast)
          final meshPacket = MeshPacketDto(
            from: 0xFFFFFFFF - channelIdx, // Pseudo sender ID based on channel
            to: 0xFFFFFFFF, // Broadcast
            channel: channelIdx,
            rxTime: DateTime.now().millisecondsSinceEpoch ~/ 1000,
            decoded: TextPayloadDto(message),
          );

          _emitEvent(
            MeshPacketEvent(packet: meshPacket, decoded: meshPacket.decoded!),
          );

          return; // Don't process as mesh packet
        } catch (e) {
          _log('Failed to parse channel message response: $e', level: 'error');
          return;
        }
      } else if (cmd == 0x83) {
        // PUSH_CODE_MSG_WAITING - indicates a message is queued
        _log('Received PUSH_CODE_MSG_WAITING (0x83)');
        // This is just a notification, nothing to parse
        return; // Don't process as mesh packet
      }
    }
