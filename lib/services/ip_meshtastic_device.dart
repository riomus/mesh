...
/// Implementation of [ChattingDevice] for IP devices.
class IpMeshtasticDevice implements ChattingDevice {
  final String _deviceId;
  final String _host;
  final int _port;
  int _currentPacketId = Random().nextInt(0xFFFFFFFF);

  IpMeshtasticDevice(this._deviceId, this._host, this._port);

  @override
  String get id => _deviceId;

  @override
  String get displayName => '$_host:$_port'; // Or fetch from device state if available
  ...
  Future<int> sendMessage(String text, int? toId, {int? channelIndex}) async {
    ...
    if (client == null) {
      ...
      // We need to call connectIp.
      // But connectToId should have returned it if connected.
      // If not connected, we try to connect.
      await DeviceStatusStore.instance.connectIp(_host, _port);
      // Try getting client again
      final retryClient = await DeviceStatusStore.instance.connectToId(
        _deviceId,
      );
      if (retryClient == null) {
        throw Exception('Could not connect to IP device $_deviceId');
      }
  ...
  Future<int> _sendMessageWithClient(
    ...
    if (channelIndex != null) {
      ...
      print(
        'IpMeshtasticDevice: sendMessage: toId is null and channelIndex is null. Not sending message.',
      );
      return 0;
    }
  ...
  Future<int> _sendTracerouteWithClient(
    ...
    print(
      'Sending traceroute packet to node $targetNodeId (IP device): $packet',
    );
