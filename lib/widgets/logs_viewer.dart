...
class LogsViewer extends StatefulWidget {
  ...
  const LogsViewer({super.key, this.stream, this.maxItems = 500, this.maxHeight});
...
class _LogsViewerState extends State<LogsViewer> {
  ...
  final Set<String> _levels = {'debug', 'info', 'warn', 'error'}; // selected levels (legacy support)
  bool _paused = false;

  @override
  void initState() {
    super.initState();
    _sub = (widget.stream ?? LoggingService.instance.listenAll()).listen((e) {
      if (_paused) return;
      setState(() {
        _all.add(e);
  ...
  Future<void> _openAddChipDialog() async {
    // Prepare defaults
    String? draftKey = _newFilterKey ?? (_seenKeys.isNotEmpty ? _seenKeys.first : null);
          ...
          builder: (context, setDlg) {
            void doAddTag() {
              ...
              if (draftOp == _ChipOp.exact) {
                final c = _ChipFilter.tag(key: key, value: draftValue.trim(), op: _ChipOp.exact);
                ...
                final c = _ChipFilter.tag(key: key, value: valueTrim, op: _ChipOp.regex);
                setState(() => _addChipUnique(c));
              }
                    ...
                    if (draftKind == _ChipKind.tag) ...[
                      ...
                      if (draftKey != null) ...[
                        Wrap(
                          ...
                          runSpacing: 6,
                          children: [
                            for (final v in (_seenValuesByKey[draftKey!] ?? const <String>{}).take(18))
                              ActionChip(
                                label: Text(v),
                                onPressed: () => setDlg(() => draftValue = v),
                              ),
                          ],
                        ),
